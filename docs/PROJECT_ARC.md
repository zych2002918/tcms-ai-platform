...
- `tcms-platform-recon-baseline`(TASK)：2026-09 实测基线——22故障/22场景/18需求/8报文/36信号/4功能/5设备；图谱221/342；向量216；8 Agent 任务硬编码防漂移；FaultLab `_profiles()` 与前端 `FAULT_SPOT_META` 双表无同步校验；testgen 的 venv editable 指向已删除 D: 路径(需 PYTHONPATH=src)。
