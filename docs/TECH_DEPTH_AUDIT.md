...
## 12. 收敛声明与新功能冻结（2026-09，尊重项目边界）
- **已达成"Agent 岗位胜任力证明"目标**：全量 pytest **205 passed + 1 skipped**、ruff clean、
  vitest 16/16、图谱交互 e2e 5/5；HARDENING 11/11 + ROI P1-a/P1-b/P2 脚本全落地；
  文档（README / HARDENING / TECH_DEPTH_AUDIT）与代码一致。
- **新功能冻结**：自本条起不再追加未承诺的功能（避免镀金越界）。技术上限/诚实边界：
  默认离线字符级检索（真语义=可选）、LLM 可选（无 key 全流程可跑）、run_scenario 需引擎
  runner 接线、单角色管线不冒充多 agent。
- **维护期范围**：只做 bug 修复、文案/措辞与文档一致性、门禁数字同步、必要时扩充 golden/e2e
  回归——不改架构、不加新能力。
