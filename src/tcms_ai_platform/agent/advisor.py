...
def _rule_reply(m: AssetModel, turn: AdvisorTurn, goal: str) -> str:
    ...
    if i == "custom_proposal":
        return (
            "你说的像是 TCMS 故障字典（22 个真实故障）还没收录的新故障，或者是"
    ...
    # out_of_domain
    return (
        "这句我没在 TCMS 测试主题里找到对应内容（我熟悉 22 个真实故障、13+ 个场景、"
        "报文/信号与安全需求）。可以试试说「车门故障不能发车」「验证超速降级」"
        "「VCU 心跳丢失怎么处置」，或直接描述你想注入/验证的故障现象。"
    )
