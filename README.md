...
- **故障演示（FaultLab）** — 把真实故障场景变成**可播放的动画**：注入 → 检测 →
  处置 → 恢复。列车状态、速度/制动缸压力曲线、事件解释随时间轴同步推进。
  不是花哨动效——事件时刻来自真实场景 YAML，处置来自真实引擎断言（或故障字典），
  每个事件都可溯源（见页内"诚实性标注"）。
- **知识图谱** — 大白话检索 TCMS 领域知识（图谱 + 向量双通道），结果分类 + 附小白解释 + 关系图谱
...
真实资产(tcms-can-test) ──loader──▶ L1 资产模型 ──FastAPI──▶ Web UI (/)
        (内置快照/活上游/兄弟目录 三级解析)
        ├──▶ 知识底座：图谱(215节点/318边) + 向量(210文档) + GraphRAG
